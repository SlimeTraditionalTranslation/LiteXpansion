...
public class AdvancedSolarPanel extends SlimefunItem implements InventoryBlock, EnergyNetProvider {
    ...
    public AdvancedSolarPanel(Type type) {
        ...
        createPreset(this, type.getItem().getItemMetaSnapshot().getDisplayName().orElse("&7太陽能板"),
            blockMenuPreset -> {
                for (int i = 0; i < 9; i++) {
    ...
    public int getGeneratedOutput(@Nonnull Location l, @Nonnull Config data) {
        ...
        if (l.getWorld().getEnvironment() == World.Environment.NETHER) {
            ...
            generationType = "&a主世界 &e(日間)";
        } else if (rate == this.type.getNightGenerationRate()) {
            generationType = "&a主世界 &8(夜間)";
        }
        ...
        if (inv.toInventory() != null && !inv.toInventory().getViewers().isEmpty()) {
            inv.replaceExistingItem(PROGRESS_SLOT,
                canGenerate ? new CustomItemStack(Material.GREEN_STAINED_GLASS_PANE, "&a發電中",
                    ...
                    : new CustomItemStack(Material.ORANGE_STAINED_GLASS_PANE, "&c未發電",
                    "", "&7發電機已達到最大儲存容量.",
                    "", "&7儲存: &6" + Utils.powerFormatAndFadeDecimals(stored) + " J")
            );
        }
